...
func BlockSave(ctx context.Context, db *mongo.Database, v *types.Block, opts ...*options.InsertOneOptions) error {
	return Save(ctx, db.Collection(BlockCollectionName), v, opts...)
}
...
func BlockFindAll(ctx context.Context, db *mongo.Database, filter bson.M, opts ...*options.FindOptions) ([]*types.Block, error) {
	var v []*types.Block
	if err := FindAll(ctx, db.Collection(BlockCollectionName), filter, &v, opts...); err != nil {
