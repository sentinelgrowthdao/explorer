...
func (q *Querier) Invoke(ctx context.Context, method string, args, reply interface{}, opts ...grpc.CallOption) error {
	...
	if heights := md.Get(grpctypes.GRPCBlockHeightHeader); len(heights) > 0 {
		...
		if req.Height < 0 {
			return fmt.Errorf("request height must be positive")
		}
	}
	...
	if err = grpcCodec.Unmarshal(res.Value, reply); err != nil {
		return err
	}
